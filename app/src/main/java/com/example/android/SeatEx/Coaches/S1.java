...
import com.example.android.SeatEx.Pair;
            ...
            buttons[i].setOnClickListener(new View.OnClickListener()
            {
                ...
                public void onClick(View view)
                {
                    ...
                    if(visited[seatNumber]==1)// pink
                    {
                        ...
                        // make them grey
                        databaseExpenses.child("Node1").child(TrainNumber).child("S"+Integer.toString(MyCoach)).child(Integer.toString(MySeat)).addListenerForSingleValueEvent(new ValueEventListener()
                        {
                            @Override
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot)
                            {
                                ...
                                databaseExpenses.child("Node1").child(TrainNumber).child("S"+Integer.toString(MyCoach)).child(Integer.toString(MySeat)).setValue(temp);
                            }
                        ...
                        databaseExpenses.child("Node1").child(TrainNumber).child("S"+Integer.toString(1)).child(Integer.toString(seatNumber)).addListenerForSingleValueEvent(new ValueEventListener()
                        {
                            @Override
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot)
                            {
                                ...
                                temp.setInterstedIn(null);
                                temp.setInterested(0);
                                databaseExpenses.child("Node1").child(TrainNumber).child("S"+Integer.toString(1)).child(Integer.toString(seatNumber)).setValue(temp);
                            }
                            ...
                            for(int j=1;j<=72;j++)
                            {
                                databaseExpenses.child("Node1").child(TrainNumber).child("S"+Integer.toString(i)).child(Integer.toString(j)).addValueEventListener(new ValueEventListener() {
                                    ...
                                    public void onDataChange(@NonNull DataSnapshot dataSnapshot)
                                    {
                                        ...
                                        temp2.remove(new Pair("S1",seatNumber));
                                        databaseExpenses.child("Node1").child(TrainNumber).child("S"+Integer.toString(temp.getCoach())).child(Integer.toString(temp.getCoach())).setValue(temp);
                                    }
                    ...
                    else if(seatDetails[seatNumber].getInterested()==1)// green color
                    {
                        ...
                        databaseExpenses.child("Node1").child(TrainNumber).child("S"+Integer.toString(MyCoach)).child(Integer.toString(MySeat)).addListenerForSingleValueEvent(new ValueEventListener()
                        {
                            ...
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot)
                            {
                                ...
                                temp2.add(new Pair(1,seatNumber));
